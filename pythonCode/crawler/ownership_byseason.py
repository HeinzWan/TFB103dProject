...
for key, value in seasons.items():
    ownership = get_data(f'''select stock_code, avg(avg_board) avg_board_bys, avg(over1000_rate) over1000_rate_bys, avg(round((100 - over400_rate),2)) under400_rate_bys 
    from ownership
    where {value}
    group by stock_code;''')
    ownership.insert(1,"Date",key)
    # print(ownership)
    engine = create_engine('mysql+pymysql://root:123456@localhost:3306/tfb103d_project')
    ownership.to_sql('ownership_afetl', engine, if_exists="append", index=False)
