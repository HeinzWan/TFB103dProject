...
for key, value in data_season.items():
    sql1 = '''select aa.iso_code, aa.location, avg(aa.new_cases), avg(aa.new_cases_smoothed) from covid19details aa
    ...
    dataa = cur.fetchall()
    for data in dataa:
        sql2 = '''INSERT INTO covidseason VALUES ('%s', '%s', '%s', '%d', '%f'); ''' \
               % (data[0], data[1], key, data[2], data[3])
        cur.execute(sql2)
        conn.commit()
conn.close()
